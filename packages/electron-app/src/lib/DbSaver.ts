import { db, NewGame, NewGameAlternateName, NewGameImage, NewPlatform, NewPlatformAlternateName } from "@/core/db";
import _ from "lodash";
import EventEmitter from "node:events";
import PQueue from "p-queue";

const mapXmlGameToNewGame = (data: any): NewGame => ({
  Name: data.Name,
...
const mapXmlPlatformToNewPlatform = (data: any): NewPlatform => ({
  Name: data.Name,
...
const mapXmlPlatformAlternateNameToNewPlatformAlternateName = (data: any): NewPlatformAlternateName => ({
  Alternate: data.Alternate,
  Name: data.Name,
});

const mapXmlGameAlternateNameToNewGameAlternateName = (data: any): NewGameAlternateName => ({
  Alternate: data.AlternateName,
  DatabaseID: data.DatabaseID,
  Region: data.Region || null,
});

const mapXmlGameImageToNewGameImage = (data: any): NewGameImage => ({
  DatabaseID: data.DatabaseID,
...
export class DbSaver extends EventEmitter<DbSaverEventMap> {
  ...
  constructor() {
    super();
    this.dbWriteQueue = new PQueue({ concurrency: 100 }); // Limit concurrent DB writes
        ...
        for (const key of currentKeys) {
          ...
          this.dbWriteQueue.add(async () => {
            try {
              ...
              if (key === 'Game') {
                await this.dbInstance.insertInto('Games').values(batch.map(mapXmlGameToNewGame))
                  .onConflict(oc => oc.column('DatabaseID').doUpdateSet(eb => ({
                    Name: eb.ref('Games.Name'), Platform: eb.ref('Games.Platform'), ReleaseDate: eb.ref('Games.ReleaseDate'), ReleaseYear: eb.ref('Games.ReleaseYear'),
                    Overview: eb.ref('Games.Overview'), MaxPlayers: eb.ref('Games.MaxPlayers'), ReleaseType: eb.ref('Games.ReleaseType'), Cooperative: eb.ref('Games.Cooperative'),
                    VideoURL: eb.ref('Games.VideoURL'), CommunityRating: eb.ref('Games.CommunityRating'), WikipediaURL: eb.ref('Games.WikipediaURL'), ESRB: eb.ref('Games.ESRB'),
                    CommunityRatingCount: eb.ref('Games.CommunityRatingCount'), Genres: eb.ref('Games.Genres'), Developer: eb.ref('Games.Developer'), Publisher: eb.ref('Games.Publisher'),
                  }))).execute();
              } else if (key === 'Platform') {
                await this.dbInstance.insertInto('Platforms').values(batch.map(mapXmlPlatformToNewPlatform))
                  .onConflict(oc => oc.column('Name').doUpdateSet(eb => ({
                    Emulated: eb.ref('Platforms.Emulated'), ReleaseDate: eb.ref('Platforms.ReleaseDate'), Developer: eb.ref('Platforms.Developer'), Manufacturer: eb.ref('Platforms.Manufacturer'),
                    Cpu: eb.ref('Platforms.Cpu'), Memory: eb.ref('Platforms.Memory'), Graphics: eb.ref('Platforms.Graphics'), Sound: eb.ref('Platforms.Sound'),
                    Display: eb.ref('Platforms.Display'), Media: eb.ref('Platforms.Media'), MaxControllers: eb.ref('Platforms.MaxControllers'), Notes: eb.ref('Platforms.Notes'),
                    Category: eb.ref('Platforms.Category'), UseMameFiles: eb.ref('Platforms.UseMameFiles'),
                  }))).execute();
              } else if (key === 'PlatformAlternateName') {
                await this.dbInstance.insertInto('PlatformAlternateNames').values(batch.map(mapXmlPlatformAlternateNameToNewPlatformAlternateName))
                  .onConflict(oc => oc.columns(['Name', 'Alternate']).doUpdateSet(eb => ({
                    Name: eb.ref('PlatformAlternateNames.Name'), Alternate: eb.ref('PlatformAlternateNames.Alternate'),
                  }))).execute();
              } else if (key === 'GameAlternateName') {
                await this.dbInstance.insertInto('GameAlternateNames').values(batch.map(mapXmlGameAlternateNameToNewGameAlternateName))
                  .onConflict(oc => oc.columns(['DatabaseID', 'Alternate']).doUpdateSet(eb => ({
                    Alternate: eb.ref('GameAlternateNames.Alternate'), Region: eb.ref('GameAlternateNames.Region'),
                  }))).execute();
              } else if (key === 'GameImage') {
                await this.dbInstance.insertInto('GameImages').values(batch.map(mapXmlGameImageToNewGameImage))
                  .onConflict(oc => oc.columns(['DatabaseID', 'FileName']).doUpdateSet(eb => ({
                    FileName: eb.ref('GameImages.FileName'), Type: eb.ref('GameImages.Type'), Region: eb.ref('GameImages.Region'), CRC32: eb.ref('GameImages.CRC32'),
                  }))).execute();
              }
            } catch (error) {
              console.error(`Error saving batch for ${key}:`, error);
              this.emit('error', error as Error); // Emit error from DbSaver
            }
